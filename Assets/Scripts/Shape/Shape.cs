...
public class Shape : MonoBehaviour, IPointerClickHandler, IPointerUpHandler, IBeginDragHandler, IDragHandler, IEndDragHandler,IPointerDownHandler
{
            ...
            for(var column = 0; column < shapeData.columns; column++)
            {
                if (shapeData.board[row].column[column])
                {
                    ...
                    _currentShape[currentIndexInList].GetComponent<RectTransform>().localPosition =
                        new Vector2(GetXPositionForShapeSquare(shapeData, column, moveDistance), GetYPositionForShapeSqaure(shapeData, row, moveDistance));
    ...
    private float GetYPositionForShapeSqaure(ShapeData shapeData, int row, Vector2 moveDistance)
    {
        float shiftOnY = 0f;

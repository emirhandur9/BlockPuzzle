...
namespace Emir
{
    public class Grid : MonoBehaviour
    {
        ...
        private void SpawnGridSquares()
        {
            ...
            for (var row = 0; row < rows; row++)
            {
                for (var column = 0; column < columns; column++)
                {
                    ...
                    _gridSquares[_gridSquares.Count - 1].GetComponent<GridSquare>().SetImage(_lineIndicator.GetGridSquareIndex(square_index) %2 == 0); // 2 image var, indexe göre bi o bi diðeri ayarlýyoruz
                    square_index++;
                }
            }
        }
        ...
        private void CheckAnyLineIsCompleted()
        {
            ...
            {
                List<int> data = new List<int>(9);//initial capacity
        ...
        private List<int[]> GetAllSquaresCombination(int shapeColumns, int shapeRows)
        {
            ...
            while(lastRowIndex +(rows -1) < 9)
            {
                ...
                {
                    for(var column = lastColumnIndex; column < lastColumnIndex + columns; columns++)
                    {
